...
const TagTemplate: React.SFC<TagTemplateProps> = props => {
	let albums = []
	let albumCount
	...
	const currTag = props.pathContext.slug
...
export const query = graphql`
	query TagTemplateQuery($slug: String!) {
		...
		albums: allMarkdownRemark(
			...
			filter: { frontmatter: { tags: { in: [$slug] }, draft: { ne: true } }, fileAbsolutePath: { regex: "/albums/" } }
		) {
			totalCount
			edges {
				node {
		...
		tracks: allMarkdownRemark(
			sort: { order: DESC, fields: [frontmatter___date] }
			filter: { frontmatter: { tags: { in: [$slug] }, draft: { ne: true } }, fileAbsolutePath: { regex: "/tracks/" } }
		) {
			totalCount
			edges {
				node {
